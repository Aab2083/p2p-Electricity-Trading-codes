...
void get_sensor_data() {
  ...
  if (currentMillis - lastMillis >= 1000) {
   ...
   // Calculate voltage at divider input
    ...
    send_current = (rawCurrent - 512) * (3.3 / 4096.0) / currentMultiplier; // Assuming ACS712 is centered at 1.65V for ESP32

    // Calculate power in Watts
    float power = send_voltage * send_current;

    // Calculate units transferred (assuming 1 unit = 1 Watt-hour)
    unitsTransferred += (power / 3600000.0);}
...
// Fetch data
sendHTTPRequest("http://192.168.104.25/mysql_esp/getdata.php", "id=esp32_03", payload);
